...
| v2.0 | 2026-03-22 | 根据审查报告完善：添加状态转换矩阵、错误处理规范、配置管理、软删除支持；精简 Dockerfile/Makefile 章节 | - |
| v2.1 | 2026-03-22 | 明确 Task-Job 绑定关系：使用 K8s Job 原生资源管理沙箱执行环境 | - |
...
| US-D06 Provider选择与配置 | Provider管理 | 4.1.1, 6.2.9 | ✓ |
| US-A01 租户管理 | 租户管理 | 6, 7.4 | ✓ |
| US-A02 任务模板管理 | 模板管理 | 4.1, 7.3 | ✓ |
| US-A03 能力注册管理 | 能力管理 | 4.1, 7.6 | ✓ |
| US-A04 Provider管理 | Provider管理 | 4.1.1, 6.2.9 | ✓ |
| US-O01 任务监控 | 监控告警 | 9 | ✓ |
...
| **故障隔离** | 单个任务失败不影响其他任务和系统稳定性 | 每个任务独立沙箱Job Pod，资源隔离 |
...
│                                                                      │
│  3. 任务执行                                                        │
│     沙箱Job Pod ──▶ Wrapper启动Claude Code CLI                      │
│             ──▶ CLI执行任务                                         │
│             ──▶ Wrapper解析输出                                     │
│             ──▶ 状态上报到控制面                                    │
│             ──▶ Log Agent采集日志到SLS                              │
...
| **沙箱Job** | 执行层 | 任务执行环境 | 控制面(API) | 可替换Agent运行时 |
...
**ProviderSvc 接口**（Agent 运行时管理）：

```go
type ProviderService interface {
    // Provider 注册与管理
    RegisterProvider(ctx context.Context, req *RegisterProviderRequest) (*Provider, error)
    ...
    TestProviderConnection(ctx context.Context, providerID string) error

    // 租户级 Provider 配置
    SetTenantDefaultProvider(ctx context.Context, tenantID string, providerID string) error
    GetTenantDefaultProvider(ctx context.Context, tenantID string) (*Provider, error)
}

// Provider 定义
type Provider struct {
    ID          string            `json:"id"`
    Name        string            `json:"name"`         // 如: "claude-code", "zhipu-coding", "cursor"
    Type        ProviderType      `json:"type"`         // claude_code, openai_compatible, custom
...
type ProviderConfig struct {
    // API 配置
    APIEndpoint    string            `json:"api_endpoint"`    // API 地址 (对应 ANTHROPIC_BASE_URL)
    APIKeyRef      string            `json:"api_key_ref"`     // K8s Secret 引用 (对应 ANTHROPIC_AUTH_TOKEN)

    // 模型映射（cc switch 风格）
    ModelMapping   *ModelMapping     `json:"model_mapping"`   // 模型名称映射
    ...
    EnabledPlugins map[string]bool   `json:"enabled_plugins"` // 启用的插件

    // 高级配置
    ExtraParams    map[string]any    `json:"extra_params"`    // 其他扩展参数
}

// 模型映射（将 Claude 模型名映射到实际模型）
type ModelMapping struct {
    DefaultModel   string `json:"default_model"`    // ANTHROPIC_MODEL
    ...
    ReasoningModel string `json:"reasoning_model"`  // ANTHROPIC_REASONING_MODEL
}

// 权限配置
type Permissions struct {
    Allow []string `json:"allow"` // 允许的操作，如 "Bash(node:*)"
    Deny  []string `json:"deny"`  // 禁止的操作
}

type ProviderType string
const (
    ProviderTypeClaudeCode       ProviderType = "claude_code"        // Claude Code CLI (官方)
    ProviderTypeAnthropicCompat  ProviderType = "anthropic_compatible" // Anthropic 兼容 API (智谱等)
    ProviderTypeOpenAICompat     ProviderType = "openai_compatible"  // OpenAI 兼容 API
    ProviderTypeCustom           ProviderType = "custom"             // 自定义
)
```

> **说明**：Provider 配置兼容 `cc switch` 格式，支持：
> - 模型名称映射（opus→glm-5, sonnet→glm-4.7 等）
> - 环境变量注入
> - 权限配置
> - 插件开关
...
│  ├── 依赖: ProviderSvc (Provider获取)                               │
│  ├── 依赖: AuditSvc (操作记录)                                      │
│  └── 依赖: NotifySvc (状态通知)                                     │
│                                                                      │
│  TemplateSvc                                                        │
│  ├── 依赖: CapabilitySvc (能力校验)                                 │
│  ├── 依赖: ProviderSvc (Provider校验)                               │
│  └── 依赖: AuditSvc (变更记录)                                      │
│                                                                      │
│  TenantSvc                                                          │
│  ├── 依赖: UserSvc (用户关联)                                       │
...
| 场景 | 操作 | 角色 |
|------|------|------|
| 管理端 | 配置系统预置 Provider | admin |
| 管理端 | 设置租户默认 Provider | admin |
| 管理端 | 查看租户下所有 Provider | admin |
| 用户端 | 查看可用 Provider 列表（系统+租户+个人） | developer |
| 用户端 | 创建个人 Provider | developer |
| 用户端 | 设置个人默认 Provider | developer |
| 创建任务 | 下拉选择 Provider | developer |
...
| 优先级 | 来源 | 说明 |
|--------|------|------|
| 1 (最高) | 环境变量 | 覆盖所有其他配置，用于敏感信息和部署时动态配置 |
| 2 | K8s ConfigMap/Secret | 集群内配置管理 |
| 3 | 配置文件 (config.yaml) | 本地开发时的默认配置 |
| 4 (最低) | 代码默认值 | 硬编码的默认值 |
...
sls:
  ...
  # 敏感配置 - 仅从环境变量
  access_key: ${SLS_ACCESS_KEY}
  access_secret: ${SLS_ACCESS_SECRET}
...
| 配置类型 | 存储方式 | 访问控制 |
|---------|---------|---------|
| 数据库密码 | K8s Secret / 环境变量 | 仅应用可访问 |
| Redis 密码 | K8s Secret / 环境变量 | 仅应用可访问 |
| SLS 凭证 | K8s Secret / 环境变量 | 仅应用可访问 |
| Git Token | K8s Secret / 环境变量 | 仅沙箱 Job Pod 可访问 |

**处理规则**：
- 敏感配置不在配置文件中存储明文
- 敏感配置不在日志中打印
- 生产环境必须通过 K8s Secret 挂载
- 配置文件中必须使用 `${ENV_VAR}` 占位符
...
> **设计决策**：MVP 阶段使用 K8s 原生 `Job` 资源管理沙箱执行环境，而非自定义 CRD。

#### 5.2.1 为什么选择 Job 而非 Pod 或 CRD

| 选项 | 优点 | 缺点 | MVP 适用性 |
|------|------|------|-----------|
| **Pod** | 简单直接 | 需手动管理重试、清理、状态追踪 | ❌ |
| **Job** ✅ | 内置重试、完成追踪、自动清理 | 暂停/恢复需要额外处理 | ✅ |
| **CRD** | 声明式、GitOps 友好 | 复杂度高，需要 Controller 开发 | ❌ v2.0+ |

#### 5.2.2 Task 与 Job 的绑定关系
...
| 配置项 | 值 | 说明 |
|--------|-----|------|
| `backoffLimit` | 0 | MVP 阶段禁用自动重试，由控制面统一管理重试逻辑 |
| `ttlSecondsAfterFinished` | 3600 | 完成 1 小时后自动清理 Pod |
| `activeDeadlineSeconds` | 按模板配置 | 任务超时时间 |
| `restartPolicy` | Never | 不自动重启，由 wrapper 控制退出 |
| `parallelism` | 1 | 单任务单 Pod |
| `completions` | 1 | 一次性任务 |

#### 5.2.4 Job 生命周期与 Task 状态同步

| Job 状态 | Task 状态 | 同步触发 |
|---------|---------|---------|
| Created | scheduled | Executor 创建 Job 成功 |
| Running | running | Pod 进入 Running 状态 |
| Complete | succeeded | Job 条件满足，wrapper 上报完成 |
| Failed | failed | Job 失败或超时 |
...
spec:
  ...
  # 或手动清理策略（可选）
  # successfulPodsHistoryLimit: 1
  # failedPodsHistoryLimit: 3
```

> **注意**：MVP 阶段使用 `ttlSecondsAfterFinished` 自动清理，后续版本可考虑保留成功 Pod 用于调试。

### 5.3 沙箱 Job 架构（Sidecar 模式）
...
cleanup() {
    ...
    echo "{\"status\": \"interrupted\", \"exit_code\": 130, \"timestamp\": $(date +%s)}" > /workspace/.agent-state/status.json
    # 如果 CLI 进程还在运行，发送 SIGTERM
...
│  cli-runner 读取:                                                   │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │  MVP 阶段采用 wrapper HTTP 接口方式：                          │ │
│  │  1. wrapper 通过 HTTP 接收干预指令                             │ │
│  │  2. wrapper 将指令写入 inject.json                             │ │
│  │  3. cli-runner 的启动脚本轮询读取 inject.json                  │ │
│  │     (每 1 秒检查一次文件变化)                                   │ │
...
```
1. Executor 创建 Job (sandbox-task-xxx)
2. Job 创建 Pod，所有容器同时启动
3. wrapper 开始发送心跳
...
| 资源超限 | K8s OOMKilled | Job 失败，标记任务失败 |
| Job 创建失败 | K8s API 错误 | 任务回到 pending，重试调度 |
...
| cli-runner → 互联网 | HTTPS | 模型API、Web Search、MCP服务等 |
| cli-runner → 企业服务 | HTTPS | 知识库检索、上下文注入、Git仓库 |
...
│      │                                  └──▶ Intervention (干预)     │
│      │                                                               │
│      └── 1:N ──▶ Template (模板) ── references ──▶ Provider          │
│                      │                                               │
│                      └── 1:N ──▶ TemplateVersion (版本)              │
│                                                                      │
│  Capability (能力) ── belongs to ──▶ Tenant (或全局)                 │
│                                                                      │
│  Provider (运行时) ── belongs to ──▶ Tenant (或全局)                 │
│      │                                                               │
│      └── referenced by ──▶ Template, Task                            │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
CREATE TABLE templates (
    ...
    -- Provider 配置
    provider_id     VARCHAR(36) COMMENT '指定Provider(NULL使用租户默认)',
...
CREATE TABLE tasks (
    ...
    -- Provider 配置
    provider_id     VARCHAR(36) NOT NULL COMMENT '使用的Provider ID',
...
#### 6.2.9 providers（Agent运行时配置表）

```sql
CREATE TABLE providers (
    id              VARCHAR(36) PRIMARY KEY COMMENT 'Provider ID',

    -- 作用域（三层：系统/租户/用户）
    scope           ENUM('system', 'tenant', 'user') NOT NULL DEFAULT 'system' COMMENT '作用域',
    tenant_id       VARCHAR(36) COMMENT '租户ID(scope=tenant或user时必填)',
    user_id         VARCHAR(36) COMMENT '用户ID(scope=user时必填)',

    -- 基本信息
    name            VARCHAR(64) NOT NULL COMMENT 'Provider名称',
    type            ENUM('claude_code', 'anthropic_compatible', 'openai_compatible', 'custom')
                    NOT NULL COMMENT 'Provider类型',
    description     TEXT COMMENT '描述',

    -- API 配置
    api_endpoint    VARCHAR(512) COMMENT 'API端点(ANTHROPIC_BASE_URL)',
    api_key_ref     VARCHAR(256) COMMENT 'API密钥引用(K8s Secret)',

    -- 模型映射 (JSON)
    model_mapping   JSON COMMENT '模型映射配置',
    -- 示例: {"default": "glm-5", "opus": "glm-5", "sonnet": "glm-4.7", "haiku": "glm-4.5-air"}

    -- 运行时配置
    runtime_type    ENUM('cli', 'api', 'sdk') DEFAULT 'cli' COMMENT '运行时类型',
    runtime_image   VARCHAR(256) COMMENT 'Docker镜像',
    runtime_command JSON COMMENT '启动命令',

    -- cc switch 兼容配置 (JSON)
    env_vars        JSON COMMENT '环境变量(cc switch env)',
    permissions     JSON COMMENT '权限配置(cc switch permissions)',
    enabled_plugins JSON COMMENT '启用的插件(cc switch enabledPlugins)',

    -- 扩展参数
    extra_params    JSON COMMENT '其他扩展参数',
    ...
    INDEX idx_type_status (type, status),
    INDEX idx_deleted (deleted_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Agent运行时配置表';
```

**Provider 作用域与可见性**：

| scope | tenant_id | user_id | 说明 | 可见范围 |
|-------|-----------|---------|------|---------|
| system | NULL | NULL | 系统预置Provider | 所有用户 |
| tenant | 租户ID | NULL | 租户级Provider | 该租户所有用户 |
| user | 租户ID | 用户ID | 用户个人Provider | 仅该用户 |
...
CREATE TABLE user_provider_defaults (
    ...
    user_id         VARCHAR(36) NOT NULL COMMENT '用户ID',
    provider_id     VARCHAR(36) NOT NULL COMMENT '默认Provider ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户默认Provider设置';
```
...
> **注意**：本节定义任务状态的合法转换路径，是任务生命周期管理的核心规则。

#### 6.3.1 状态转换矩阵

| 从状态 | 可转换到 | 触发条件 | 说明 |
|--------|---------|---------|------|
| pending | scheduled | 调度器取出任务 | 任务进入调度队列 |
| pending | cancelled | 用户取消 | 任务尚未开始执行 |
| scheduled | running | Pod 启动成功 | 任务开始执行 |
| scheduled | failed | Pod 创建失败 | 资源不足或配置错误 |
| running | paused | 用户暂停 | 人工干预 |
| running | waiting_approval | 审批断点 (v1.1+) | 到达审批节点 |
| running | succeeded | 任务完成 | 执行成功 |
| running | failed | 任务执行失败 | 错误或超时 |
| running | cancelled | 用户取消 | 人工干预 |
| paused | running | 用户恢复 | 人工干预 |
| paused | cancelled | 用户取消 | 人工干预 |
| waiting_approval | running | 审批通过 (v1.1+) | 继续执行 |
| waiting_approval | cancelled | 审批拒绝 (v1.1+) | 终止任务 |
| failed | retrying | 自动重试触发 | 系统自动 |
| retrying | running | 重试开始 | 重新执行 |
| retrying | failed | 重试次数耗尽 | 最终失败 |
...
└────┬────┘         └─────┬─────┘            └────┬────┘           │
     ...
     ▼                    ▼                       ▼                 ▼
┌──────────┐         ┌─────────┐           ┌───────────┐     ┌─────────┐
│cancelled │         │ failed  │◀──────────│ succeeded │     │ paused  │
└──────────┘         └────┬────┘           └───────────┘     └────┬────┘
                          │                                      │
                          │ 重试                                 │ 恢复/取消
                          ▼                                      │
...
| 终态 | 说明 | 后续操作 |
|------|------|---------|
| succeeded | 任务执行成功 | 可查看结果、产物 |
| failed | 任务执行失败（重试耗尽） | 可查看错误信息、手动重试 |
| cancelled | 任务被取消 | 可查看取消原因 |
...
#### 7.1.6 Provider管理 `/api/v1/providers`

| 方法 | 路径 | 说明 | 角色 |
|------|------|------|------|
| GET | /providers | Provider列表（系统+租户+用户） | developer, admin |
| GET | /providers/:id | Provider详情 | developer, admin |
| POST | /providers | 创建Provider | admin, developer |
| PUT | /providers/:id | 更新Provider | admin, developer(自己的) |
| DELETE | /providers/:id | 删除Provider | admin, developer(自己的) |
| POST | /providers/:id/test | 测试Provider连接 | admin, developer |
| GET | /providers/system | 系统预置Provider列表 | all |
| GET | /providers/available | 可用Provider列表(任务创建时) | developer |

**租户/用户默认Provider设置**：

| 方法 | 路径 | 说明 | 角色 |
|------|------|------|------|
| GET | /tenants/:id/providers/default | 获取租户默认Provider | admin |
| PUT | /tenants/:id/providers/default | 设置租户默认Provider | admin |
| GET | /users/me/providers/default | 获取用户默认Provider | developer |
| PUT | /users/me/providers/default | 设置用户默认Provider | developer |

**Provider 配置示例**（兼容 cc switch 格式）：

```json
{
    "name": "zhipu-glm",
    "type": "anthropic_compatible",
    "description": "智谱 GLM 模型（Anthropic 兼容）",
    "api_endpoint": "https://open.bigmodel.cn/api/anthropic",
    "api_key_ref": "zhipu-api-key",
    "model_mapping": {
        "default": "glm-5",
...
| 错误类型 | 重试 | 最大次数 | 退避策略 | 说明 |
|---------|------|---------|---------|------|
| 网络超时 | 是 | 3 | 指数退避 (1s→2s→4s) | 临时网络问题 |
| 限流 (429) | 是 | 5 | 固定 + 抖动 (1s±0.5s) | 服务端限流 |
| 服务不可用 (503) | 是 | 3 | 指数退避 | 服务暂时不可用 |
| 参数校验失败 (400) | 否 | - | - | 客户端错误 |
| 认证失败 (401) | 否 | - | - | 凭证无效 |
| 权限不足 (403) | 否 | - | - | 权限错误 |
| 资源不存在 (404) | 否 | - | - | 资源已删除 |
...
| 操作类型 | 幂等键 | TTL | 说明 |
|---------|--------|-----|------|
| POST /tasks | `X-Idempotency-Key` Header | 24h | 防止重复创建任务 |
| POST /tasks/:id/inject | `X-Idempotency-Key` Header | 1h | 防止重复注入指令 |
| 其他 POST 操作 | `X-Idempotency-Key` Header | 1h | 建议使用 |

**幂等性实现**：
```
1. 客户端生成唯一幂等键 (UUID)
2. 请求携带 X-Idempotency-Key Header
3. 服务端检查 Redis 中是否存在该 Key
   - 存在：返回缓存的响应
   - 不存在：执行请求，缓存响应，设置 TTL
```

#### 7.6.3 熔断器配置

| 保护对象 | 阈值 | 熔断时间 | 半开请求数 | 说明 |
|---------|------|---------|-----------|------|
| 数据库查询 | 10s 内 5 次失败 | 30s | 1 | 数据库保护 |
| Redis 操作 | 10s 内 5 次失败 | 30s | 1 | 缓存保护 |
| K8s API | 10s 内 3 次失败 | 60s | 1 | K8s 保护 |
| 外部 API | 10s 内 3 次失败 | 60s | 1 | 外部服务保护 |

#### 7.6.4 降级策略

| 场景 | 降级策略 | 说明 |
|------|---------|------|
| 数据库不可用 | 返回缓存数据 / 服务降级 | 仅影响查询操作 |
| Redis 不可用 | 直接查询数据库 | 性能下降，功能正常 |
| K8s API 不可用 | 拒绝新任务创建 | 已运行任务不受影响 |
| SLS 不可用 | 本地日志缓冲 | 异步重试上报 |
...
| sandbox | Job | sandbox-{task-id} | 动态 | 按模板配置，创建 Pod 执行任务 |
...
| 沙箱Job Pod | 2核4G ~ 4核8G | 由 Job 创建，按任务模板配置 |
...
│  控制面Pod ──▶ K8s API Server ──▶ 创建/管理沙箱Job                 │
│  沙箱Job Pod ──▶ control-plane-svc ──▶ 回调上报                    │
│  沙箱Job Pod ──▶ 互联网 (模型API、Web Search、MCP等)                │
│  沙箱Job Pod ──▶ 企业服务 (知识库、上下文注入、Git仓库)              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 方向 | 目标 | 策略 | 说明 |
|------|------|------|------|
| 出站 | 控制面 Service | 允许 | 心跳、状态上报 |
| 出站 | 互联网 (0.0.0.0/0) | 允许 | 模型API、Web Search、Git、MCP服务等 |
| 出站 | 企业知识库 | 允许 | 上下文注入、知识检索 |
| 出站 | 其他公司内网 | 禁止 | 安全隔离 |
| 入站 | 控制面 | 允许 | 干预指令 (wrapper HTTP API) |

**企业内部服务访问配置**：

| 服务类型 | 访问方式 | 说明 |
|---------|---------|------|
| 企业知识库 | 内网域名/Service | RAG 检索、文档查询 |
| 上下文注入服务 | 内网域名/Service | 任务相关的上下文数据 |
| Git 仓库 (行云) | 内网域名或公网 | 代码克隆/推送 |

> **注意**：
> - 企业内部服务通过白名单配置，而非全量开放
> - MVP 阶段可通过 ConfigMap 配置允许访问的内网服务列表
> - 后续版本可通过 Service Mesh 实现更细粒度的访问控制
...
| Git凭证 | K8s Secret加密 | 仅沙箱Job Pod可访问 |
...
| 任务隔离 | 每个任务独立沙箱Job Pod |
| 网络访问 | 沙箱Job Pod：允许互联网 + 白名单内网服务（知识库、上下文注入、Git） |
...
> **说明**：完整的 Dockerfile 请参考 `deploy/dockerfiles/` 目录。本节仅描述镜像规格要求。

#### 11.2.1 镜像规格总览

| 镜像 | 基础镜像 | 主要组件 | 大小目标 | 安全要求 |
|------|---------|---------|---------|---------|
| control-plane | alpine:3.19 | Go 二进制 | <30MB | 非 root, healthcheck |
| frontend | nginx:alpine | React 构建产物 | <50MB | 仅静态文件 |
| cli-runner | ubuntu:22.04 | Claude CLI, git, dev tools | <500MB | 沙箱隔离 |
| agent-wrapper | alpine:3.19 | Go 二进制 | <20MB | 非 root, healthcheck |
| log-agent | 官方镜像 | 日志采集 | ~100MB | Fluent Bit / Filebeat |

#### 11.2.2 构建要求

| 要求 | 说明 |
|------|------|
| 多阶段构建 | 所有自研镜像使用多阶段构建，减小最终镜像大小 |
| 非 root 用户 | 所有镜像以 UID 1000 的非 root 用户运行 |
| 健康检查 | 所有服务镜像包含 HEALTHCHECK 指令 |
| 漏洞扫描 | CI 中使用 Trivy 扫描镜像漏洞，高危漏洞阻断部署 |
| 最小化基础镜像 | 优先使用 alpine 基础镜像 |

#### 11.2.3 镜像构建命令

```bash
# 构建所有镜像
make docker-build-all VERSION=v1.0.0

# 推送到仓库
make docker-push VERSION=v1.0.0

# 单独构建
make docker-build-control-plane VERSION=v1.0.0
make docker-build-frontend VERSION=v1.0.0
make docker-build-cli-runner VERSION=v1.0.0
make docker-build-wrapper VERSION=v1.0.0
```

> **实际 Dockerfile 文件位置**：`deploy/dockerfiles/`
...
| 命令 | 用途 | 说明 |
|------|------|------|
| `make build` | 构建控制面服务 | 输出到 `bin/control-plane` |
| `make run` | 本地运行 | 使用 `configs/local.yaml` |
| `make test` | 运行单元测试 | 含覆盖率报告 |
| `make lint` | 代码检查 | golangci-lint |
| `make clean` | 清理构建产物 | 删除 `bin/` 目录 |

#### 11.3.2 Docker 命令

| 命令 | 用途 |
|------|------|
| `make docker-build-all VERSION=v1.0.0` | 构建所有镜像 |
| `make docker-push VERSION=v1.0.0` | 推送所有镜像到仓库 |
| `make docker-build-control-plane` | 单独构建控制面镜像 |
| `make docker-build-frontend` | 单独构建前端镜像 |
| `make docker-build-cli-runner` | 单独构建 CLI Runner 镜像 |
| `make docker-build-wrapper` | 单独构建 Wrapper 镜像 |
...
| `make db-rollback` | 回滚数据库迁移 |

> **实际 Makefile 文件位置**：`Makefile`
...
| Executor | Job管理逻辑 | Go testing + mock |
...
| `UNAUTHORIZED` | 401 | 未授权访问（API Key 无效或过期） |
| `FORBIDDEN` | 403 | 权限不足 |
| `NOT_FOUND` | 404 | 资源不存在 |
| `CONFLICT` | 409 | 资源冲突（如重复创建） |
| `QUOTA_EXCEEDED` | 429 | 配额超限 |
| `INTERNAL_ERROR` | 500 | 内部服务错误 |
| `SERVICE_UNAVAILABLE` | 503 | 服务不可用（如 Provider 不可用） |

#### 13.2.2 业务错误码

| 错误码 | 说明 | 场景 |
|--------|------|------|
| `TASK_NOT_FOUND` | 任务不存在 | 查询或操作不存在的任务 |
| `TASK_STATUS_INVALID` | 任务状态不允许此操作 | 对已完成的任务执行暂停 |
| `TEMPLATE_NOT_FOUND` | 模板不存在 | 引用不存在的模板 |
| `TEMPLATE_VALIDATION_FAILED` | 模板校验失败 | 模板 YAML 格式错误 |
| `TEMPLATE_NOT_PUBLISHED` | 模板未发布 | 使用草稿状态的模板创建任务 |
| `TENANT_NOT_FOUND` | 租户不存在 | 租户 ID 无效 |
| `TENANT_SUSPENDED` | 租户已暂停 | 对暂停租户执行操作 |
| `USER_NOT_FOUND` | 用户不存在 | 用户 ID 无效 |
| `USER_DISABLED` | 用户已禁用 | 操作已禁用的用户 |
| `PROVIDER_NOT_FOUND` | Provider 不存在 | Provider ID 无效 |
| `PROVIDER_UNAVAILABLE` | Provider 不可用 | Provider 连接测试失败 |
| `CAPABILITY_NOT_FOUND` | 能力不存在 | 能力 ID 无效 |
| `CAPABILITY_NOT_AUTHORIZED` | 能力未授权 | 租户未授权使用该能力 |
| `QUOTA_CPU_EXCEEDED` | CPU 配额超限 | 租户 CPU 使用超过配额 |
| `QUOTA_MEMORY_EXCEEDED` | 内存配额超限 | 租户内存使用超过配额 |
| `QUOTA_CONCURRENCY_EXCEEDED` | 并发任务数超限 | 租户并发任务超过配额 |
| `QUOTA_DAILY_TASKS_EXCEEDED` | 每日任务数超限 | 租户每日任务超过配额 |
| `QUOTA_TOKEN_EXCEEDED` | Token 配额超限 | 单任务 Token 超过配额 |
...
```json
{
    "error": {
        "code": "TASK_NOT_FOUND",
        "message": "Task with ID 'task-xxx' not found",
        "details": {
            "task_id": "task-xxx"
        }
    }
