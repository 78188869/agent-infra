...
## Scope
- [ ] 更新 config.local.yaml 使用 SQLite 替代 MySQL
- [ ] main.go 支持 APP_ENV 自动选择配置文件
- [ ] 集成 miniredis 作为本地内存 Redis
- [ ] 串联 Scheduler/Executor 初始化到 main.go
- [ ] 调用 SeedProviders 填充初始数据
- [ ] 添加优雅关停（graceful shutdown）
- [ ] 添加 `make local` 命令
- [ ] 集成测试验证一键启动
- [ ] 文档说明本地开发步骤
...
## Acceptance Criteria
- [ ] `make local` 单个命令即可启动完整应用
- [ ] 启动后 API 可完成完整的任务生命周期操作
- [ ] 无需预先安装 MySQL、Redis、K8s
- [ ] 有文档说明本地开发的启动步骤和依赖要求
