...
func (s *TaskScheduler) Dequeue(ctx context.Context) (*DequeuedTask, error) {
	for {
		...
		if s.getTask != nil {
			...
			// getTask callback is required for Dequeue operation
			s.limiter.Release(ctx, item.TenantID)
			s.queue.Enqueue(ctx, item)
			return nil, fmt.Errorf("getTask callback is required for Dequeue operation")
		}
...
func (s *TaskScheduler) Preempt(ctx context.Context, taskID string, status string, progress int) error {
	...
	// getTask callback is required for Preempt operation
	if s.getTask == nil {
		return fmt.Errorf("getTask callback is required for Preempt operation")
	}
